...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'posts' | 'bookmarks' | 'approvals'>('posts');
  ...
  const handleQuickSwitchUser = (targetUsername: string) => {
    ...
    if (target) {
      ...
      addToast(`Switched account to "${target.username}" (${target.status === 'approved' ? 'Released' : 'Pending'})`, 'success');
    } else {
      addToast(`User "${targetUsername}" not found.`, 'error');
    }
  };
    ...
    <div className={`min-h-screen flex flex-col font-sans antialiased transition-colors ${
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 py-6">
        ...
        {(activeTab === 'posts' || activeTab === 'bookmarks') && (
          <div className="space-y-6">
            ...
            {/* Header Toolbar */}
            <div className={`flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-4 rounded-2xl shadow-lg border transition-colors ${
              ...
              <div className="flex items-center gap-2">
                {activeTab === 'posts' ? (
                  <>
                    ...
                    <h2 className={`font-bold text-base ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>SZ Portal Nachrichten Feed</h2>
                  </>
                ) : (
                  <>
                    <Bookmark className="w-5 h-5 text-indigo-500" />
                    <h2 className={`font-bold text-base ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>Saved Bookmarks</h2>
                  </>
                )}
                <span className={`text-xs font-normal ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>({filteredPosts.length} updates)</span>
              </div>
              ...
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  placeholder="Search posts, files, or tags..."
              ...
              {/* Create post button for Phillip Dev on mobile/feed header */}
              {isPhillipDev && (
                <button
            ...
            {filteredPosts.length === 0 ? (
              <div className={`border rounded-2xl p-10 text-center ${
                ...
                <h3 className={`text-base font-semibold ${isLight ? 'text-slate-800' : 'text-slate-200'}`}>
                  {activeTab === 'bookmarks' ? 'No Bookmarks Saved Yet' : 'No Posts Found'}
                </h3>
        ...
        {/* Tab View 2: Approval Management Panel (Phillip Dev Only) */}
        {activeTab === 'approvals' && (
          <ApprovalPanel
            currentUser={currentUser}
      ...
      <footer className={`border-t py-6 text-center text-xs mt-12 transition-colors ${
        ...
        <div className="max-w-5xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <p>
            Release Policy Active • Password: <span className="font-mono text-indigo-500 font-semibold">Ingolstadt 2015</span>
          </p>
        </div>
      </footer>
      ...
      <LoginRegisterModal
        ...
        onSuccess={(user) => {
          setCurrentUserLocal(user);
          refreshState();
          addToast(`Signed in as "${user.username}"`, 'success');
        }}
      />
